...
Bank::Bank(const std::vector<Account*>& accounts) {
    accounts_ = accounts;
}
...
std::function<void(Account*)> Bank::print_balance = [] (Account* account) 
        { 
            std::cout << account->name << "'s balance: " << account->balance << std::endl;
        };
...
std::function<double(double, Account*)> Bank::accumulate_sum = [] (double sum, Account* account) {
